...
```
Host filesystem:
/Users/kunal/projects/
├── my-project/              ← Contains .git directory
│   └── .git/
│       └── worktrees/
│           └── feature-a/
...
Container filesystem (default mount):
/workspaces/
└── feature-a/               ← Only this is mounted
    └── .git  (points to /Users/kunal/projects/my-project/... → DOESN'T EXIST!)
```

**Why this is a problem:**

Without access to Git metadata, you can't:

- Run `git status`, `git log`, `git commit`, or any Git command
- Use VS Code's Git integration
- Let AI agents commit changes or create branches
- Verify what branch you're on

The worktree becomes a dead checkout.
...
→ Run `bash .devcontainer/smoke-test.sh` to see an automated verification.

...
- This allows you to run multiple AI agents in parallel without them interfering with each other.
...
- Making Git worktrees work with Dev Containers can be tricky because the worktree's .git file points to a path that isn't mounted inside the container by default.
- Without a proper template, setting up multiple worktrees and containers can be a manual and error-prone process, especially for developers who are new to either technology.
...
- Follow the instructions in [`.devcontainer/readme.md`](./.devcontainer/readme.md) to set up your worktrees and containers.
